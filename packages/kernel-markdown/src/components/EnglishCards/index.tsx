...
import {
  ...
  normalizeWordKey,
} from '#markdown/components/EnglishCards/data';
import type {
  EnglishCardExample,
  EnglishCardDetail,
  EnglishCardItem,
...
const EnglishCardContent = (props: {
  ...
  return (
    <div className="markdown-english-card-content">
      {apiState ? <EnglishCardOpenApiStatus state={apiState} /> : null}

      <section className="markdown-english-card-section markdown-english-card-section--translation">
        <div className="markdown-english-card-label">中文翻译</div>
        {item.translation ? (
          renderLines(item.translation, 'translation')
        ) : (
          <p className="markdown-english-card-line markdown-english-card-line--empty">
      ...
      {item.explanation ? (
        <section className="markdown-english-card-section markdown-english-card-section--definition">
          <div className="markdown-english-card-label">英文释义</div>
          {renderLines(item.explanation, 'explanation')}
        </section>
      ) : null}

      {item.example ? (
        <section className="markdown-english-card-section markdown-english-card-section--examples">
          <div className="markdown-english-card-label">例句</div>
          {renderExamples(item.example, item.word)}
        </section>
      ) : null}

      {item.details?.length ? (
        <section className="markdown-english-card-section markdown-english-card-section--details">
          <div className="markdown-english-card-label">拓展</div>
          {renderDetails(item.details)}
        </section>
      ) : null}

      {item.note || item.tags?.length ? (
        <div className="markdown-english-card-meta">
      ...
      <div className="markdown-english-card-actions">
...
const EnglishCardResourceLink = (props: { resource: EnglishCardResource }) => {
  const { resource } = props;

  return (
    <span className="markdown-english-card-resource">
      <a
        href={resource.href}
        className="markdown-english-card-link"
        target="_blank"
        rel="noreferrer"
      >
        <ExternalLinkIcon />
      ...
      <span className="markdown-english-card-resource-card" role="tooltip">
        <span className="markdown-english-card-resource-title">
          {resource.title ?? resource.label}
        </span>
        {resource.description ? (
          <span className="markdown-english-card-resource-description">
            {resource.description}
          </span>
        ) : null}
        <span className="markdown-english-card-resource-url">
...
const EnglishCardPractice = (props: {
  ...
  return (
    <div className="markdown-english-card-practice">
      <label
        className="markdown-english-card-practice-label"
        htmlFor={item.word}
      >
        默写中文翻译
      </label>
      <div className="markdown-english-card-practice-row">
        <input
          id={item.word}
          className="markdown-english-card-practice-input"
          type="text"
          value={answer}
        ...
        <button
          type="button"
          className="markdown-english-card-practice-check"
          onClick={onCheck}
        >
          校验
        </button>
      ...
      <div
        className={[
          'markdown-english-card-practice-result',
          `markdown-english-card-practice-result--${result}`,
        ].join(' ')}
      >
        {result === 'correct'
          ? '正确'
...
const EnglishCardOpenApiStatus = (props: { state: OpenApiWordState }) => {
  ...
  return (
    <div
      className={[
        'markdown-english-card-openapi',
        `markdown-english-card-openapi--${state.status}`,
      ].join(' ')}
    >
      <InfoCircledIcon />
      <span>{copy}</span>
...
const renderLines = (
  ...
  return lines.map((line, index) => (
    <p key={`${keyPrefix}-${index}`} className="markdown-english-card-line">
      {line}
    </p>
  ));
};
...
const renderExamples = (
  ...
  return examples.map((example, index) => {
    if (!isExampleObject(example)) {
      return (
        <div key={`example-${index}`} className="markdown-english-card-example">
          <p className="markdown-english-card-line">
    ...
    return (
      <div key={`example-${index}`} className="markdown-english-card-example">
        ...
        {example.translation ? (
          <p className="markdown-english-card-example-translation">
            {example.translation}
          </p>
        ) : null}
      </div>
...
const renderDetails = (details: Array<EnglishCardDetail>) => {
  return (
    <div className="markdown-english-card-details">
      {details.map((detail, index) => (
        <div key={`detail-${index}`} className="markdown-english-card-detail">
          <div className="markdown-english-card-detail-label">
            {detail.label}
          </div>
          <div className="markdown-english-card-detail-items">
              ...
              return (
                <span key={`detail-item-${itemIndex}`}>
                  ...
                  {speakTextValue ? (
                    <button
                      type="button"
                      className="markdown-english-card-detail-speak"
...
const renderExampleText = (value: ReactNode, word: string) => {
  ...
  return parts.map((part, index) => {
    ...
    return (
      <mark
        key={`${part}-${index}`}
        className="markdown-english-card-example-word"
      >
        {part}
      </mark>
    );
...
const isPracticeAnswerCorrect = (answer: string, item: EnglishCardItem) => {
...
const speakEnglishCardWord = (item: EnglishCardItem) => {
  if (item.audioUrl) {
...
export function MdxEnglishCards(props: EnglishCardsProps) {
  ...
  return (
    <section
      className={[
        'markdown-english-cards',
        `markdown-english-cards--${mode}`,
        className,
      ]
        .filter(Boolean)
        .join(' ')}
    >
      <div className="markdown-english-cards-header">
        <div>
          {title ? (
            <h3 className="markdown-english-cards-title">{title}</h3>
          ) : null}
          <p className="markdown-english-cards-summary">
            {items.length} 个词条
          </p>
        </div>
        <button
          type="button"
          className="markdown-english-cards-mode"
          onClick={toggleMode}
        >
      ...
      <div className="markdown-english-cards-list">
        {items.map((item, index) => {
          ...
          return (
            <article
              key={wordKey}
              className={[
                'markdown-english-card',
                isExpanded ? 'markdown-english-card--expanded' : undefined,
              ]
                .filter(Boolean)
                .join(' ')}
            >
              <div className="markdown-english-card-head">
                <div>
                  <div className="markdown-english-card-word-row">
                    <h4 className="markdown-english-card-word">
                      {cardItem.word}
                    </h4>
                    {cardItem.partOfSpeech ? (
                      <span className="markdown-english-card-part">
                        {cardItem.partOfSpeech}
                      </span>
                    ) : null}
                    {cardItem.phonetic ? (
                      <span className="markdown-english-card-phonetic">
                        {cardItem.phonetic}
                      </span>
                    ) : null}
                    <button
                      type="button"
                      className="markdown-english-card-speak"
                ...
                <div className="markdown-english-card-head-actions">
                  {isPracticeMode ? (
                    <button
                      type="button"
                      className="markdown-english-card-reveal"
                  ...
                  <button
                    type="button"
                    className="markdown-english-card-collapse"
