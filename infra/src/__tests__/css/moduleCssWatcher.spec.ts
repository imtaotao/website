...
vi.mock('#infra/css/production/moduleCssBuilder', () => ({
...
import type { InfraConfig, InfraLogger } from '#infra/types';

describe('ModuleCssWatcher', () => {
  let tempRoot: string;
  let logger: Required<InfraLogger>;

  beforeEach(() => {
    vi.useFakeTimers();
    tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'infra-watcher-'));
    logger = {
      log: vi.fn(),
      info: vi.fn(),
      error: vi.fn(),
  ...
  test('watches configured source root and infra config file', async () => {
    ...
    expect(mocks.watch).toHaveBeenCalledWith(
      [path.join(tempRoot, 'source'), path.join(tempRoot, 'infra.config.ts')],
      {
        ignoreInitial: true,
        interval: 300,
        usePolling: true,
    ...
    expect(logger.log).toHaveBeenCalledWith('[infra:css] watch mode ready');
