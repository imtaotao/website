import type {
  BlogEnglishCardItem,
  BlogEnglishCardsOpenApiConfig,
} from '#blog/components/MarkdownTypes';

export type NormalizedOpenApiConfig = Required<
  Pick<BlogEnglishCardsOpenApiConfig, 'enabled' | 'endpoint' | 'language'>
>;
...
const DICTIONARY_CACHE_PREFIX = 'blog:english-card:dictionary:v1';
...
export function normalizeOpenApiConfig(
  openApi: boolean | BlogEnglishCardsOpenApiConfig | undefined,
...
): Promise<Partial<BlogEnglishCardItem>> {
...
const readCache = (
  word: string,
  config: NormalizedOpenApiConfig,
): Partial<BlogEnglishCardItem> | undefined => {
