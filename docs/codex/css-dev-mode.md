...
这个文档记录 `auk build-css` 后续补充 Vite 开发模式支持时的设计方向。目标是让 `app` 在开发环境中直接消费 `packages` 源码 CSS，并保持和生产环境 CSS 构建产物一致的语义。
...
- 开发环境不写入 `dist`。
- CSS dev 解析尽量复用 `auk build-css` 的规则，避免开发和生产语义分叉。
...
- `auklet.config.ts` 变化：会影响外部 CSS 入口，可先 full reload。
...
- 优先抽取 `auk build-css` 中“生成 CSS 入口图”的纯逻辑，构建命令负责写文件，Vite 插件负责返回虚拟 CSS code。
- 可复用能力包括：
  - `auklet.config.ts` 读取。
  - `fileWalker` 和样式文件筛选。
...
1. 抽出 CSS 入口图生成逻辑，不改变现有 `auk build-css` 行为。
2. 增加 Vite 插件，支持 `style.css`、`external.css` 和模块级 `*.css` 的虚拟入口。
3. 在 dev server 中监听 `packages/kernel-*/src/**/*.{ts,tsx,css,less}` 和 `packages/kernel-*/auklet.config.ts`。
