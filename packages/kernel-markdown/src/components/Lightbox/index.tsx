...
import type { LightboxImage } from '#markdown/components/Types';
import '#markdown/components/Lightbox/index.css';
type LightboxProps = {
  image: LightboxImage;
  onClose: () => void;
  onPrev?: () => void;
  ...
  useEffect(() => {
    ...
    setImageMotionClassName(
      transitionDirection > 0
        ? 'markdown-lightbox-image--slide-next'
        : 'markdown-lightbox-image--slide-prev',
    );
  ...
  return (
    ...
    >
      {props.onPrev ? (
        <button
          type="button"
          className="markdown-lightbox-nav markdown-lightbox-nav--prev"
          onClick={(event) => {
      ...
      {props.onNext ? (
        <button
          type="button"
          className="markdown-lightbox-nav markdown-lightbox-nav--next"
          onClick={(event) => {
            event.stopPropagation();
            props.onNext?.();
          }}
      ...
      <figure
        className="markdown-lightbox-figure"
      ...
      >
        <img
          src={props.image.src}
          alt={props.image.alt}
          className={`markdown-lightbox-image ${imageMotionClassName}`.trim()}
          decoding="async"
          fetchPriority="high"
          onClick={handleImageClick}
        />
        {props.image.caption ? (
          <figcaption className="markdown-lightbox-caption">
            {props.image.caption}
          </figcaption>
        ) : null}
      </figure>
