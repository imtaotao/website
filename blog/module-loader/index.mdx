...
前段时间系统性地看了一遍模块加载相关的内容，顺手写了一个实验性质的实现 [liberty](https://github.com/imtaotao/liberty)。它的目标很明确，就是在浏览器端模拟出一套 **CJS** 的模块加载体验。当然后续我也做了 [esModule](https://github.com/imtaotao/virtual-es-module) 的 `polyfill`，原理上和这个差不多，这里就不展开了。
...
这点非常关键：缓存不能等执行结束后再写入，而要在执行前先占位。否则一旦出现循环引用，整个行为就和 CJS 不一致了。
