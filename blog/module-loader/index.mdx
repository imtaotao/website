---
title: 浏览器端 CJS 模块加载器
slug: module-loader
publishedAt: 2019-06-20
summary: 以一个浏览器端类 CJS 模块加载器为例，记录模块缓存、插件机制、同步加载和执行隔离的设计思路。
coverPosition: center 30%
tags:
  - frontend
---

前段时间系统性地看了一遍模块加载相关的内容，顺手写了一个实验性质的实现 <GitHubRepo repo="imtaotao/liberty" />。它的目标很明确，就是在浏览器端模拟出一套 **CJS** 的模块加载体验。
...
如果想模拟 CJS，最基本的几样东西不能少：`require`，`module`，`exports`，`__filename`，`__dirname`。其中最关键的一点是：`require` 返回的模块最好保持同步语义，这样用起来才真正像 CJS。
...
一开始我基本就是按这条线推进的。后面为了处理更多实际问题，细节才越来越多。
...
<Callout tone="warning" title="边界说明">
  这种实现并不以安全性为目标。这里讨论的是原理，不是生产级隔离沙箱。
</Callout>
...
<Callout tone="warning" title="缓存 key 的边界">
  但光有规范化还不够。因为真实请求过程中，URL
  还可能经历重定向，所以最终又加了一层 `responseURL`
  级别的缓存，尽量用服务端真正返回的资源地址来兜底。
</Callout>
...
<Callout tone="tip" title="插件职责">
  对我来说，这套设计最重要的不是“功能多强”，而是足够清楚：核心加载流程只负责调度，真正的内容转换交给插件。
</Callout>

## JavaScript 模块怎么执行

对 `.js` 文件，最终还是要落到**如何执行源码**这一步。在 Node.js 里，类似能力可以借助 `vm.runInThisContext`：
...
<Callout tone="warning" title="执行方案取舍">
  `eval` 直接排除，成本和可维护性都不理想。`new Function` 比 `eval`
  好一些，但我并不想让全部模块都走这种路径。最后采用的是动态创建 `script`
  标签同步执行代码，再结合 sourcemap 辅助定位源码。
</Callout>
...
<Callout tone="warning" title="循环引用处理">
  这点非常关键：缓存不能等执行结束后再写入，而要在执行前先占位。否则一旦出现循环引用，整个行为就和
  CJS 不一致了。
</Callout>
...
<Callout tone="warning" title="失败状态清理">
...
1. 静态资源是通过请求拿到的文件内容
2. 模块结果是资源经过插件系统处理之后的产物

在 CJS 环境里，我们更习惯关注模块缓存；但如果放到浏览器里，为了处理同步语义、依赖预取和性能问题，资源本身也值得缓存。

## 同步加载为什么困难

<Callout tone="warning" title="同步加载难点">
...
<Callout tone="tip" title="ESM 带来的启发">
...
<Callout tone="warning" title="HTTP 层面的限制">
...
<Callout title="实验价值">对我来说，这种实验的价值就在这里。</Callout>

1. 怎么统一同步和异步加载
2. 怎么降低重复执行风险
3. 怎么把内容转换能力从核心里抽离出来
4. 怎么在浏览器里模拟接近 CJS 的体验
